...
import { TikiTopple } from './game.js';
import './style.css';
...
function GameRoom({G,ctx,player,room,name,isMyTurn,client,messages,chatRef,chatText,setChatText,sendChat,quickSend,currentID,rulesOpen,setRulesOpen,link,leaveRoom}) {
  ...
  const doMove=()=>{if(picked===null||toastTiki===null||!isMyTurn)return; client.moves.PlayCard(picked, toastTiki); setPicked(null);setToastTiki(null)};
  ...
  return <main className="game-screen">
    ...
    <div className="game-layout"><section className="table-column">
      ...
      {G?.phase==='playing'&&<div className="hand-zone"><div className="hand-header"><span><b>你的手牌</b> <small>{me?.hand?.length||0} 張</small></span><button onClick={()=>setShowSecret(v=>!v)}>{showSecret?'隱藏祕密圖騰':'查看祕密圖騰'} ◉</button></div>{showSecret&&<div className="secret-callout">你的祕密圖騰：{me?.secret?.map((id,index)=>`${['🥇','🥈','🥉'][index]} ${G.board.find(t=>t.id===id)?.name||'🌴'}`).join('　')}</div>}<div className="card-fan">{me?.hand?.map((card,index)=><button className={`action-card ${picked===index?'picked':''}`} key={`${card}-${index}`} style={{'--tilt':`${(index-(me.hand.length-1)/2)*3}deg`,'--order':index}} onClick={()=>setPicked(index)} disabled={!isMyTurn} title="選取行動牌"><span className="card-spark">✳</span><b>{card==='topple'?'推倒':card==='toast'?'爆破':`上移 ${card.slice(2)}`}</b><i>{card==='topple'?'↓↓':card==='toast'?'✹':`↑${card.slice(2)}`}</i><small>{card==='topple'?'TOPPLE':card==='toast'?'TOAST':'TIKI UP'}</small></button>)}</div><div className="play-row"><span>{isMyTurn?(picked===null?'選一張牌，再點選要移動的圖騰':toastTiki===null?'選擇場上的圖騰':`已選擇 ${G.board.find(t=>t.id===toastTiki)?.name||'圖騰'}`):'等待你的回合'}</span><button className="play-button" disabled={!isMyTurn||picked===null||toastTiki===null} onClick={doMove}>出牌並執行 →</button></div></div>}
...
function TikiScene({board=[],decorative=false,lastMove,interactive=false,secret=[],onTikiClick}) {
  ...
  useEffect(()=>{
    ...
    const sync=()=>{
      const data=dataRef.current;const source=decorative&&data.board.length===0?Array.from({length:9},(_,id)=>({id,active:true})):data.board;const live=source.filter(t=>t.active); const signature=live.map(t=>t.id).join(',')+`:${data.secret.join(',')}`;
      ...
      live.forEach((t,index)=>{const secretOn=data.secret.includes(t.id);const idol=makeTiki(secretOn?'#f0c763':'#bd9060',decorative?.78:.60);const target=new THREE.Vector3((index-(live.length-1)/2)*1.04,.46,.06);const start=oldPositions.get(t.id)?.clone()||target.clone();idol.position.copy(start);idol.rotation.y=(index%2?-.12:.12);idol.userData={id:t.id,start,target,moveStart:performance.now()};idolRoot.add(idol);activeRef.current.push(idol)});
    };
    sync();
    let down=null;
