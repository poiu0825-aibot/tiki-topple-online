...
function App() {
  ...
  function finishJoinProfile() {
    ...
    if(gameState?.G?.phase==='lobby')client?.moves.SetProfile({name:finalName,avatarTikiId:joinProfile.avatarTikiId,ready:true});
...
function GameRoom({G,ctx,player,room,name,joining,isMyTurn:turnOwned,client,messages,chatRef,chatText,setChatText,sendChat,quickSend,currentID,rulesOpen,setRulesOpen,link,leaveRoom}) {
  ...
  useEffect(()=>{ if(!G||!me||!client||G.phase!=='lobby')return; client.moves.SetProfile({name, avatarTikiId:me.avatarTikiId, ready:!joining}); },[name,joining,me?.avatarTikiId,me?.joined,G?.phase]);
  ...
  const targetAllowed=selectedCard==='toast' ? (me?.roundPlays||0)>0&&active.length>0 : Boolean(standingHead)&&!(selectedCard?.startsWith('up')&&standingIndex<Number(selectedCard.slice(2)));
  ...
  const selectionHint=selectedCard?.startsWith('up')?`這張牌要站在第 ${Number(selectedCard.slice(2))+1} 位或更後方。`:selectedCard==='toast'?'按開始行動即可爆破最下方的不倒翁。':selectedCard==='swap'?'第一張依站立位置決定；再選第二張。':'目標依目前站立位置決定。';
  const selectHead=(id)=>{
    ...
    <div className="game-layout"><section className="table-column">
      ...
      {G?.phase==='playing'&&!me?.joined&&<div className="idle-restart-banner" role="status">你已因累積逾時離開本場。<button onClick={leaveRoom}>返回首頁</button></div>}
       <div className={`board-wrap ${G?.phase==='playing'?'is-playing':''}`} onClickCapture={e=>{if(!swapChoosing)return;if(e.target instanceof Element&&e.target.closest('.scene-swap-picker, .scene-card-dock'))return;setPicked(null);setSwapFirst(null);setSwapTarget(null);e.stopPropagation()}}>
         ...
         {swapChoosing&&<div className="scene-swap-picker" role="group" aria-label="場上不倒翁，依序選擇兩個頭像"><div className="scene-swap-title"><b>場上不倒翁</b><span>{swapFirst===null?'請選第一個頭像':swapTarget===null?'請選第二個頭像':'已選好兩個頭像'}</span></div><div className="scene-swap-grid">{active.map((tiki,index)=>{const selected=swapFirst===tiki.id?1:swapTarget===tiki.id?2:0;return <button key={tiki.id} className={`scene-swap-head ${selected?'selected':''}`} aria-pressed={Boolean(selected)} aria-label={`${tiki.name}，第 ${index+1} 位${selected?`，第 ${selected} 個已選取`:''}`} onClick={()=>chooseSwapHead(tiki.id)}><TikiPortrait tiki={DISPLAY_TIKIS[tiki.id]} size={42}/><small>{index+1}</small>{selected>0&&<i>{selected}</i>}</button>})}</div>{selectedCardValid&&<button className="scene-action-button swap-confirm" onClick={doMove}>開始行動 →</button>}</div>}
         {SHRINE_MODE&&G?.phase==='playing'&&!swapChoosing&&<div className="scene-card-dock">{cardFan}{isMyTurn&&selectedCard&&<button className="scene-action-button" disabled={!selectedCardValid} onClick={doMove} title={!selectedCardValid?selectionHint:'開始行動'}>開始行動 →</button>}</div>}
       ...
       {G?.phase==='lobby'&&<div className="pre-game-panel"><div className="lobby-title"><b>等待玩家入場</b><small>玩家顏色已自動分配</small></div><span className="lobby-status">{sortedPlayers.length<2?`等待至少 2 位玩家 · ${sortedPlayers.length}/2`:sortedPlayers.some(p=>!p.profileReady)?'等待玩家設定姓名':'已加入 '+sortedPlayers.length+' 位'}</span>{isHost?<button className="start-button" onClick={start} disabled={sortedPlayers.length<2||sortedPlayers.some(p=>!p.profileReady)}>開始遊戲 →</button>:<span className="host-note">等待房主…</span>}</div>}
       {G?.phase==='lobby'&&SHRINE_MODE&&<div className="lobby-avatar-panel"><b>選你的不倒翁頭像</b><div className="avatar-chooser">{SHRINE_TIKIS.map((tiki,id)=><button key={id} aria-label={`頭像 ${id+1}`} aria-pressed={me?.avatarTikiId===id} className={me?.avatarTikiId===id?'selected':''} onClick={()=>client.moves.SetProfile({name,avatarTikiId:id,ready:!joining})}><img src={`/assets/daruma-head-v2-${String(id).padStart(2,'0')}.png`} alt=""/></button>)}</div></div>}
      ...
      {!SHRINE_MODE&&G?.phase==='playing'&&<div className="hand-zone">
        ...
        <div className="play-row"><span>{!isMyTurn?'等待 '+(G?.players?.[Number(currentID)]?.name||'玩家')+' 出牌':picked===null?SHRINE_MODE?'先爬上樓梯，再選一張手牌':'先靠近人頭，再選一張手牌':selectedCard==='toast'?selectionHint:!standingHead?SHRINE_MODE?'請爬到要操作的頭像旁':'請走到要操作的人頭旁':!targetAllowed?selectionHint:selectedCard==='swap'&&!selectedCardValid?'請選第二張人頭；第一張依站立位置決定':SHRINE_MODE?'已可行動；射擊、踹梯或烏鴉可能改變你的高度':'已可行動；若被推擠，操作人頭會跟著改變'}</span><button className="play-button" disabled={!isMyTurn||picked===null||!selectedCardValid} onClick={doMove}>確定行動 →</button></div>
        </>}
      </div>}
    </section>
