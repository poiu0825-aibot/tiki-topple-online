...
import { createDarumaTower } from './darumaTower.js';
...
function label(text) {
  ...
  const ctx = canvas.getContext('2d'); ctx.fillStyle = '#173b32'; ctx.beginPath(); ctx.roundRect(5, 5, 502, 118, 22); ctx.fill();
  ctx.strokeStyle = '#d8bb7d'; ctx.lineWidth = 5; ctx.stroke(); ctx.fillStyle = '#f9edcf';
  ctx.font = 'bold 43px sans-serif'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.fillText(String(text).slice(0, 23), 256, 64, 470);
  const map = new THREE.CanvasTexture(canvas); map.colorSpace = THREE.SRGBColorSpace;
  const sprite = new THREE.Sprite(new THREE.SpriteMaterial({ map, transparent: true, opacity: .72, depthTest: false }));
  sprite.scale.set(1.55, .39, 1); sprite.renderOrder = 15; return sprite;
}
...
function person(color) {
  ...
  add(new THREE.SphereGeometry(.13, 14, 12), skin, 0, .68, 0);
  add(new THREE.CylinderGeometry(.12, .11, .32, 10), suit, 0, .41, 0);
  const arms = [], legs = [];
  for (const side of [-1, 1]) {
    const arm = add(new THREE.CylinderGeometry(.035, .035, .31, 8), ink, side * .16, .39, 0); arms.push(arm);
  ...
  body.userData = { arms, legs, hitbox, label: null, labelKey: '' };
  return body;
}

export default function ShrineScene({ board = [], players = [], myPlayerID, currentPlayer, lastMove, lastAttack, lastCrow, messages = [], backdrop = 0, shootMode = false, flashTikiId = null, flashToken = 0, onHead, onPlayer, onStairs, onGround, onWalk, interactive = true }) {
  const mount = useRef(null);
  const latest = useRef({ board, players, myPlayerID, currentPlayer, lastMove, lastAttack, lastCrow, messages, shootMode, flashTikiId, flashToken, onHead, onPlayer, onStairs, onGround, onWalk });
  latest.current = { board, players, myPlayerID, currentPlayer, lastMove, lastAttack, lastCrow, messages, shootMode, flashTikiId, flashToken, onHead, onPlayer, onStairs, onGround, onWalk };
  ...
  useEffect(() => {
    ...
    const scoreboard=new THREE.Mesh(new THREE.PlaneGeometry(2.65,2.45),new THREE.MeshBasicMaterial({map:scoreboardTexture(players,backdrop),transparent:true,side:THREE.DoubleSide}));
    scoreboard.position.set(-2.45,3.9,-1.91);scene.add(scoreboard);
    ...
    let frame = 0, scoreKey='', blastStamp=null, blastAt=0, seenFlashToken=0;
    const render = () => { frame = requestAnimationFrame(render); const data = latest.current; const now = performance.now();
      ...
      avatars.forEach((avatar, id) => { const p = data.players[id]; avatar.visible = Boolean(p?.joined); if (!avatar.visible) return;
        ...
        const pos = step ? { x: step.x + .13, y: step.y + .16, z: step.z + .24 } : { x: p.position?.x??groundStart[id][0], y: 0, z: p.position?.z??groundStart[id][1] };
        ...
        const stride = moving ? Math.sin(now * .016 + id) * .54 : 0; avatar.userData.legs[0].rotation.x = stride; avatar.userData.legs[1].rotation.x = -stride; avatar.userData.arms[0].rotation.x = -stride; avatar.userData.arms[1].rotation.x = stride;
        ...
        const shown = `${p.name || `玩家 ${id+1}`}${chosen ? ` |${rank+1} ${TIKIS[chosen.id].name}` : ''}`;
        if (shown !== avatar.userData.labelKey) { if (avatar.userData.label) { avatar.remove(avatar.userData.label); avatar.userData.label.material.map.dispose(); avatar.userData.label.material.dispose(); } avatar.userData.label = label(shown); avatar.userData.label.position.y = .98; avatar.add(avatar.userData.label); avatar.userData.labelKey = shown; }
    ...
    return () => { cancelAnimationFrame(frame); clearInterval(walkTimer);window.removeEventListener('keydown',keyDown);window.removeEventListener('keyup',keyUp); observer.disconnect(); tower.dispose(); renderer.domElement.removeEventListener('pointerdown', down); renderer.domElement.removeEventListener('pointermove', move); renderer.domElement.removeEventListener('pointerup', up); renderer.domElement.removeEventListener('wheel', wheel);scoreboard.material.map.dispose(); scene.traverse(object => { object.geometry?.dispose(); if (Array.isArray(object.material)) object.material.forEach(m => m.dispose()); else object.material?.dispose(); }); renderer.dispose(); node.removeChild(renderer.domElement); };
