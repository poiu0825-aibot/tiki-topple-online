...
export const TikiTopple = {
  ...
  setup: ({ ctx, random }, setupData) => ({
    ...
    board: shuffled(TIKIS.map((name, id) => ({ id, name, active: true })), random),
    ...
    stages: {
      ...
      play: { moves: {
        PlayCard: ({ G, playerID, ctx, events }, cardIndex, tikiId) => {
          ...
          if (!p || G.phase !== 'playing' || playerID !== ctx.currentPlayer) return;
          ...
          if (card.startsWith('up')) {
            ...
            if (index === 0) return;
            const destination = Math.max(0, index - amount);
...
function beginRound(G, ctx, random, events, first) {
  ...
  G.board = shuffled(TIKIS.map((name, id) => ({ id, name, active: true })), random);
